...
h10s_usgs.sort()
h10s_list = pd.Series(h10s_usgs)

for curhuc in h10s_usgs[10:]: 
    try:
        print(curhuc)
        ...
        with open(log_path, 'w') as f:
            ...
            temp_df, no_st_values, st_db_path = stp.create_database_file(temp_files, cur_comids, date_col = 'tim.date', proj_path = proj_path, 
                                           db_type = 'stream_temperature', add_cols = None, overwrite = overwrite)
    ...
    #%%        
            ...
            for e in epochs:
                ...
                stp.create_index(gpkg_path, layer_name, 'HUC12')
                stp.add_view(gpkg_path, layer_name, 'HUC12_boundaries', 'HUC12', 'HUC12')
            out_anoms = [s.replace('SWE', 'SwS') for s in anom_covs]
            ...
            rs_project = Project(
                   ...
                   meta_data=MetaData([
                       ...
                       Meta('Legacy HUC ID(s)', [int(h) for h in legacy_h10s]),
        ...
        try:
            ...
            with open(os.path.join(outdir, 'upload_log_retro.txt'), 'a') as f:
                if res.stdout[-14:-6]=='COMPLETE':
                    ...
                    with open(os.path.join(outdir, 'upload_list.txt'), 'a') as l:
                        l.write(f'\n{curhuc}\t{h10name_hr}\t ')
